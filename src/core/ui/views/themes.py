        ...
        try:
            ...
            for base in base_urls:
                try:
                    ...
                    last_err = exc
            raise last_err or RuntimeError("无法下载主题文件")
        except Exception as exc:
            ContentDialog(
                parent=self.window(),
                title="安装失败",
...
class ThemeViewer(ViewBase, QMainWindow):
    ...
    def _build_header(self, root: QVBoxLayout, t: dict[str, str]) -> None:
        ...
        self._header_info = QLabel("安装主题前请备份配置。")
        ...
        self.restore_button = _make_btn("还原", "default", slot=self._restore_config)
    ...
    def _check_ready(self) -> None:
        ...
        if self._load_error is not None:
            self.splash_screen.show_message(f"无法加载主题。\n{self._load_error}")
            return
    ...
    def _rebuild_list(self, items: list[dict]) -> None:
        ...
        self.count_label.setText(f"显示 {shown}/{total} 个主题" if shown < total else f"{total} 个主题")
    ...
    def _restore_config(self):
        self.restore_button.setText("正在还原…")
        ...
        try:
            ...
            self.restore_button.setText("还原完成！")
            _run_yasbc("start")
            QTimer.singleShot(2000, lambda: self.restore_button.setText("还原"))
        except Exception as e:
            ContentDialog(
                parent=self,
                title="还原失败",
